...
本仓库的 `mrdoudou-custom` 分支保存个人二开版本，`main` 分支保持原仓库最新内容。

二开内容包括：

- 多 Agent 按端口追加安装、登记和隔离卸载；
- 主/备隧道固定身份，主备独立健康检查与故障重连；
- 主线路故障时只切换业务绑定，不交换 `tun_main` / `tun_backup` 对象；
- 新增实例从 `/opt/proxy_lite/.latest` 使用最新 Agent 代码，已有实例文件不覆盖；
- 节点统计兼容双隧道上报与旧版汇总格式。

部署前请在 Cloudflare Worker 环境变量和 D1 绑定中配置自己的值，不要把令牌或密码提交到仓库。
