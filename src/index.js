...
<body class="min-h-screen bg-[#090E17] text-slate-300 relative overflow-x-hidden selection:bg-indigo-500/30">
            ...
            <div class="flex flex-col gap-3 w-full md:w-auto">
                <div class="bg-slate-900/80 backdrop-blur-md border border-slate-700/50 rounded-xl overflow-hidden shadow-lg">
                    ...
                    <div class="flex flex-wrap items-center gap-2 p-3 bg-[#0D1117]">
                        ...
                        <button onclick="copyText(document.getElementById('install-command').textContent, '\u5B89\u88C5\u547D\u4EE4\u5DF2\u590D\u5236')" class="shrink-0 px-3 py-1.5 rounded-md bg-emerald-500/15 border border-emerald-500/30 text-emerald-300 text-xs hover:bg-emerald-500/25">\u590D\u5236</button>
                    </div>
                </div>

                <div class="bg-slate-900/80 backdrop-blur-md border border-rose-900/40 rounded-xl overflow-hidden shadow-lg">
                    <div class="bg-rose-950/30 px-4 py-2 border-b border-rose-900/40 flex items-center gap-2">
                        <span class="text-xs text-rose-300 font-mono">\u5378\u8F7D VPS Agent\uFF08Root\uFF09</span>
                    </div>
                    <div class="flex flex-wrap items-center gap-2 p-3 bg-[#0D1117]">
                        <input id="uninstall-port-input" value="7920" inputmode="numeric" placeholder="要卸载的端口" class="w-28 rounded-md border border-slate-700 bg-slate-950 px-2 py-1.5 text-xs text-slate-200">
                        <code id="uninstall-command" class="hidden"></code>
                        <button onclick="copyText(document.getElementById('uninstall-command').textContent, '\u5378\u8F7D\u547D\u4EE4\u5DF2\u590D\u5236')" class="shrink-0 px-3 py-1.5 rounded-md bg-rose-500/15 border border-rose-500/30 text-rose-300 text-xs hover:bg-rose-500/25">\u590D\u5236</button>
                    </div>
                </div>
    ...
    <script>
        ...
        function updateAgentCommands() {
            ...
            const uninstallPort = document.getElementById('uninstall-port-input')?.value.trim() || '7920';
            ...
            document.getElementById('uninstall-command').textContent = 'curl -fsSL -u sysadmin "' + '${domain}' + '/remove-agent?ip=' + encodeURIComponent(ip) + '&port=' + encodeURIComponent(uninstallPort) + '" | bash';
        }
        ...
        ['agent-ip-input', 'agent-port-input', 'uninstall-port-input'].forEach(id => {
